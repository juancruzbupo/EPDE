...
import type { PropertyCertificateData, PropertyReportData } from '../types/dashboard';
...
export function createPropertyQueries(apiClient: AxiosInstance) {
  return {
    ...
    async getPropertyHealthIndex(
      id: string,
      signal?: AbortSignal,
    ): Promise<ApiResponse<import('../types/dashboard').PropertyHealthIndex>> {
    ...
    async getPropertyCertificate(
      id: string,
      signal?: AbortSignal,
    ): Promise<ApiResponse<PropertyCertificateData>> {
      const { data } = await apiClient.get(`/properties/${id}/certificate`, { signal });
      return data;
    },
  };
}
