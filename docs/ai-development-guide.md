...
55. **Dashboard invalidation en TODA mutación que afecte conteos** — useCreate/useUpdate/useDelete de plans, properties, tasks, budgets, service-requests DEBEN llamar `invalidateDashboard(queryClient)` (web) o `invalidateClientDashboard(qc)` (mobile). Incluye analytics key en mobile
56. **Server pages con `getServerUser()`** — Todas las detail pages `[id]/page.tsx` DEBEN llamar `getServerUser()` para consistencia, aunque la API ya enforce auth. El patrón estándar es `Promise.all([serverFetch(...), getServerUser()])`
