...
205. **SIEMPRE #109 (presupuesto vs solicitud de servicio — definición canónica)**: Estos dos conceptos se explican en 6 ubicaciones (glosario, PageHeader help ×2, inline helpers ×2, web dialog, mobile dialog). TODOS deben decir lo mismo. La regla que los distingue: **¿el usuario sabe qué necesita?** Sí → Presupuesto ("ya sabés qué reparación necesitás y querés saber cuánto cuesta", ej: canaletas rotas, pintar fachada). No → Solicitud de servicio ("detectaste un problema pero no sabés qué hacer, EPDE lo evalúa", ej: humedad en la pared, grieta en el techo). NUNCA invertir los ejemplos ni mezclar las definiciones. Referencia canónica: `packages/shared/src/constants/glossary.ts`
