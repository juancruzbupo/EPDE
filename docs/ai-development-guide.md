...
189. **SIEMPRE #93 (soft-delete en nested includes)**: La Prisma extension en `prisma.service.ts` solo filtra `deletedAt: null` en operaciones ROOT de los 8 modelos declarados. No aplica a relaciones cargadas via `include: { relation: true }`. Al usar `include` sobre cualquier modelo con campo `deletedAt` (task, category, taskNote, budgetComment, etc.), agregar `where: { deletedAt: null }` al include explícitamente. Ver `apps/api/src/prisma/soft-delete-include.ts` para constantes reutilizables y la lista completa de modelos afectados
