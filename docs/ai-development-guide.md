...
4. **Repository pattern** — Solo los repositorios inyectan `PrismaService`. Los services inyectan repositorios. Enforzado por dos guardrails: `no-restricted-imports` (bloquea el import de `prisma.service` en `*.service.ts`) y la ESLint rule custom `local/no-prisma-in-service` (bloquea cualquier `this.prisma.*` adentro de `*.service.ts`). Allowlist de excepciones documentadas en [eslint-rules/no-prisma-in-service.mjs](../eslint-rules/no-prisma-in-service.mjs): hoy solo `auth-audit.service.ts` (audit log append-only sin ownership check). Agregar a esa lista requiere un rationale de una línea + referencia desde acá
