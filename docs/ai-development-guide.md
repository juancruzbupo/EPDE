...
188. **SIEMPRE #92 (error throwing por capa)**: Las excepciones HTTP (`BadRequestException`, `NotFoundException`, `ForbiddenException`) solo se lanzan en **Controllers** o en domain exception classes (`apps/api/src/common/exceptions/`). Los **Services** y **Repositories** NUNCA importan `@nestjs/common` para lanzar HTTP errors directamente — lanzan domain errors tipados (ej. `BudgetNotQuotableError`) que el controller o el GlobalExceptionFilter mapea a HTTP. Esto mantiene los services testeables sin levantar NestJS, y deja la capa HTTP solo en el borde del sistema
189. **SIEMPRE #93 (soft-delete en nested includes)**: La Prisma extension en `prisma.service.ts` solo filtra `deletedAt: null` en operaciones ROOT de los 8 modelos declarados (ver ADR-009). No aplica a relaciones cargadas via `include: { relation: true }`. Al usar `include` sobre cualquier modelo con campo `deletedAt` (task, category, taskNote, budgetComment, etc.), agregar `where: { deletedAt: null }` al include explícitamente. Ver `apps/api/src/prisma/soft-delete-include.ts` para constantes reutilizables: `TASKS_INCLUDE_WHERE` (para includes de tasks desde plan/property), `TASK_NOTES_INCLUDE_WHERE` (para taskNotes en findWithDetails)
