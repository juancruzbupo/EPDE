...
4. **Repository pattern** — Solo los repositorios inyectan `PrismaService`. Los services inyectan repositorios. Enforzado por dos guardrails: `no-restricted-imports` (bloquea el import de `prisma.service` en `*.service.ts`) y la ESLint rule custom `local/no-prisma-in-service` (bloquea cualquier `this.prisma.*` adentro de `*.service.ts`). Para cruzar el boundary transaccional sin romper la regla, usar `this.<repo>.withTransaction(fn)` (definido en `BaseRepository`) en lugar de `this.prisma.$transaction`. Allowlist de excepciones documentadas en [eslint-rules/no-prisma-in-service.mjs](../eslint-rules/no-prisma-in-service.mjs): (1) `auth-audit.service.ts` — audit log append-only sin ownership check; (2) `metrics-collector.service.ts` — consulta `pg_stat_activity` (Postgres system view), no hay entity ni soft-delete que proteger. Agregar a esa lista requiere un rationale de una línea en el archivo de la rule + referencia desde acá
...
86. **Bulk task creation via `POST /maintenance-plans/:id/tasks/bulk`** — Recibe `categoryTemplateId`, crea las TaskTemplates como tareas del plan en un `createMany`. Salta tareas cuyo nombre ya existe en el plan (case-insensitive, whole-plan match). Responde `{ created, skipped, skippedNames }` — NO un count. Admin-only, throttled, Zod-validated. Es endpoint de agregado INCREMENTAL (sector nuevo post-creación, ej. pool); la creación inicial del plan va por `POST /inspections/:checklistId/generate-plan`. Frontend: dropdown "Agregar desde plantilla" en PlanStatusBar (solo aparece con `hasTasks`) abre dialog con checkboxes multi-select; `useBulkAddTasks` en `use-task-operations.ts` muestra toast con ambos counts + primeras 3 omitidas, invalida plans + dashboard
...
198. **SIEMPRE #102 (mobile font scale via `useType()`)**: Screens mobile que muestran texto al CLIENT deben importar `const TYPE = useType()` de `@/lib/fonts` en lugar del `import { TYPE }` estático. `useType()` multiplica fontSize + lineHeight por el scale que el usuario eligió en Perfil → Tamaño de texto (sm/base/lg/xl). Screens no migrados siguen usando `TYPE` directo y renderizan al scale base — no rompen, solo no responden a la preferencia. Migrar al menos las tabs principales (dashboard, tasks, properties). Constantes de scale viven en `@epde/shared/constants/font-scale.ts` para parity con web
199. **SIEMPRE #103 (mobile confirmaciones destructivas via `confirm()`)**: Acciones destructivas o de credenciales en mobile (delete, logout, archivar plan, aprobar/rechazar presupuesto, descartar cambios sin guardar) DEBEN usar `confirm()` de `@/lib/confirm`, NO `Alert.alert`. `confirm()` retorna `Promise<boolean>` y fuerza `confirmLabel` explícito (no permite "OK"). `<ConfirmHost />` está en root layout. `Alert.alert` queda solo para informativos no-destructivos (errores, permisos, info) y selectores multi-opción (cámara vs galería)
200. **SIEMPRE #104 (`FirstTimeBanner` para onboarding contextual mobile)**: Para explicar conceptos nuevos (ISV, risk score, plan) en una pantalla mobile, usar `<FirstTimeBanner id="screen-concept" title="..." message="..." emoji="..." />`. Se muestra una sola vez, persiste dismissal en AsyncStorage con key `epde-first-time-banner-<id>`. No es sustituto de HelpHint (que es on-demand por término). Usar sparingly — si la pantalla necesita 2+ banners, el problema es el diseño de la pantalla, no la falta de banners
201. **SIEMPRE #105 (repos sin `BaseRepository` necesitan JSDoc)**: La ESLint rule `local/no-repository-without-justification` falla en CI si un archivo `*.repository.ts` en `apps/api/src/` contiene una class `*Repository` que NO extiende `BaseRepository` y NO tiene un JSDoc block citando una categoría de ADR-011 (`cross-model`, `no-base-model`, `append-only`, `sub-recurso`, o `ADR-011`). Si el repo es una excepción legítima de plumbing (ej. `FailedNotificationRepository`), agregar su basename a `ALLOWED_SKIP` en la rule
202. **SIEMPRE #106 (motivationStyle en mobile)**: El toggle `rewards` vs `minimal` vive en `stores/motivation-store.ts` (Zustand + AsyncStorage). Hooks de mutación que disparan confetti, mensajes motivacionales o alerts de savings DEBEN gatear con `useMotivationStore(s => s.motivationStyle === 'rewards')`. En modo `minimal`: toast neutro sin confetti, savings alert como `toast.info` no-blocking. Haptics.success() NO se gatea (es feedback táctil, no celebración)
203. **SIEMPRE #107 (engagement cards profesionales)**: Los cards de engagement mobile (`MonthlySummaryCard`, `ProtectedHomeBanner`, `PreventionROICard`) usan tono de **informe de gestión**: labels uppercase, números alineados a la derecha, sin emojis, sin "desafío"/"logro". Copy patrimonial: "reparaciones correctivas cuestan 8x-15x más". El dopamine trigger es evidencia de patrimonio protegido, no gamification
204. **SIEMPRE #108 (weekly challenge = "objetivo semanal")**: En web, el `WeeklyChallengeCard` usa "Objetivo semanal" / "Objetivo cumplido", NO "Desafío" ni 🎯. Tono profesional, no gamification. Card gateada por `showRewards`
205. **SIEMPRE #109 (presupuesto vs solicitud de servicio — definición canónica)**: Estos dos conceptos se explican en 7 ubicaciones (glosario, PageHeader help ×2, inline helpers ×2, web dialog, mobile dialog). TODOS deben decir lo mismo. La regla que los distingue: **¿es mantenimiento?** Sí → Solicitud de servicio ("pedí que EPDE ejecute una tarea de mantenimiento por vos: algo del plan, algo que requiere profesional, o un problema nuevo"). No → Presupuesto ("cotización para un trabajo de arquitectura fuera del mantenimiento: ampliación, renovación, plano municipal, habilitación, relevamiento"). NUNCA mezclar las definiciones. Referencia canónica: `docs/presupuestos-vs-servicios.md` + `packages/shared/src/constants/glossary.ts`
