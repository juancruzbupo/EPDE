...
| `handleReferralMilestoneReached` | `convertReferral` post-tx cuando el delta cruza un nuevo hito (1/2/3/5/10)       | Referente                              | `userId`, `userEmail`, `userName`, `milestone`, `creditMonths`, `nextMilestone`, `hasAnnualDiagnosis`, `hasBiannualDiagnosis`. Dedupe vía `jobId = referralMilestone:<email>:<milestone>` |
| `handleReferralMaxReached`       | `convertReferral` post-tx cuando la conversión es la #10 (única vez por cliente) | Admin (`ADMIN_NOTIFICATION_EMAIL` env) | `adminEmail`, `clientId`, `clientName`, `clientEmail`. Dedupe vía `jobId = referralMaxAdmin:<clientId>`                                                                                   |
