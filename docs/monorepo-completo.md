...
### ESLint rules custom (8)

`local/no-prisma-in-service` (services no inyectan PrismaService directo) · `local/no-tx-without-soft-delete-filter` (transacciones sobre soft-deletable models filtran `deletedAt: null`) · `local/no-soft-deletable-include-without-filter` (nested includes filtran) · `local/mobile-query-requires-stale-time` (mobile useQuery requiere staleTime explícito) · `local/no-inline-risk-threshold` (comparaciones de risk score van por `getRiskLevel()`) · `local/repository-override-must-be-documented` (overrides de BaseRepository requieren JSDoc) · `local/api-factory-must-exist` (`apps/*/src/lib/api/*.ts` consume `createXxxQueries(apiClient)` o está en exception list) · `local/no-repository-without-justification` (repos que no extienden BaseRepository requieren JSDoc con categoría ADR-011). Más `max-lines: 150` para hooks. Tabla completa en ADR-012.
