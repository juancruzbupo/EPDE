...
| `JWT_SECRET`                  | Si        | —                | Clave para firmar JWTs. ≥32 chars. Generar con `openssl rand -hex 32`. **Startup rechaza placeholders conocidos** (lista en `apps/api/src/config/config.module.ts:KNOWN_WEAK_JWT_SECRETS`). Rotación: ver `SECURITY.md`.                                              |
