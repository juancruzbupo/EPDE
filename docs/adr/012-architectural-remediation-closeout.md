...
| `local/repository-override-must-be-documented`   | `apps/api/src/**/*.repository.ts`                                                       | Método con `override` en repo que extiende `BaseRepository` sin JSDoc block arriba. Combina con `tsconfig.noImplicitOverride` para forzar override consciente + documentado. Cierra audit C3.                     |
| `local/no-repository-without-justification`      | `apps/api/src/**/*.repository.ts`                                                       | Class `*Repository` que no extiende `BaseRepository` y no tiene JSDoc citando una categoría de ADR-011. Cierra el gap del re-audit round 3 (3 repos habían landeado sin header).                                  |
| `local/api-factory-must-exist`                   | `apps/web/src/lib/api/**/*.ts`, `apps/mobile/src/lib/api/**/*.ts`                       | Archivo en `lib/api/` que no consume `createXxxQueries(apiClient)` de `@epde/shared` y no está en la exception list. Frena drift donde un nuevo dominio se agrega sin factory shared. Cierra audit drift-zone D3. |
| `local/no-hardcoded-routes`                      | `apps/web/src/**/*.{ts,tsx}`, `apps/mobile/src/**/*.{ts,tsx}`                           | Detecta strings de ruta hardcodeados (`/properties/123`, `/budgets`) — usar `ROUTES.*` de `lib/routes.ts`. Severity: warn (guardrail para código futuro, codebase actual ya migrado).                             |
| `max-lines: 150`                                 | `apps/web/src/hooks/use-*.ts` y `apps/mobile/src/hooks/use-*.ts` (excl. split variants) | Hook combinado que debe splittearse en `*-queries.ts` + `*-mutations.ts`. Cierra SIEMPRE #43. Mobile activado en 2026-04-15 — split de use-budgets, use-service-requests, use-task-operations.                    |
...
| `invalidate-dashboard-parity.test.ts` | Keys invalidadas por `invalidateDashboard()` en web vs mobile                              | Si una plataforma actualiza la lista y la otra no, el dashboard queda stale. Cachó divergencia web/mobile al agregar una query nueva.                                                         |
| `soft-deletable-models-sync.test.ts`  | `SOFT_DELETABLE_MODELS` en `prisma.service.ts` vs `no-tx-without-soft-delete-filter.mjs`   | Agregar un modelo soft-deletable al runtime extension sin actualizar la lista del ESLint rule deja el rule ciego. Se ejercitó al sumar `professional` en ADR-018.                             |
| `schema-constraints-parity.test.ts`   | `@db.VarChar(N)` de Prisma vs `.max(N)` de Zod schemas                                     | Si Prisma aumenta un largo sin reflejarlo en Zod, la API rechaza input válido. Si Zod permite más que Prisma, los writes fallan silenciosamente o truncan.                                    |
...
2. ~~**Split mobile hooks**~~ — DONE 2026-04-15 (audit M1/I1). Los 3 hooks que excedían 150 LOC fueron splitteados al patrón `use-X.ts` (barrel) + `use-X-queries.ts` + `use-X-mutations.ts`. La justificación previa ("mobile combina por diseño, un solo rol") era débil — fricción de cross-platform reading > beneficio de archivo único. Mobile ahora tiene el mismo `max-lines: 150` enforzado en eslint config.
...
| ESLint rules custom                    | 3                | 9                                 | +6                                                     |
| Drift tests (parity)                   | 1 (superficial)  | 7 (value-level)                   | +6                                                     |
